...
for i in {1..10}; do
  echo "Búsqueda $i"
  curl -X POST "https://publications-publishers-calculations-act.trycloudflare.com/embeddings/search_image?top_k=10" \
    -F "file=@test.jpg"
  echo ""
done
```
...
```bash
curl -X POST "https://publications-publishers-calculations-act.trycloudflare.com/fix-embeddings/regenerate-all"
```
